...
Legend: ✅ DONE · 🟡 WIP · ⬜ TODO · ⛔ BLOCKED · ✂️ CUT
...
Columns: # | Task | Files | Owner | Status | Deps | Done means | Notes. The lock lives in Notes (`WIP 2026-09-03 14:10 Stephen`); the Status cell carries the legend symbol and the word (`🟡 WIP`).
...
|---|---|---|---|---|---|---|---|
| 0.1 | Verify identities (`gcloud auth list`, `gh api user`, Devpost whoami all one person); create GCP project `hold-2026`; link billing (trial or card); enable Cloud Run, Cloud Build, Artifact Registry, Secret Manager, Vertex AI; create the Workload Identity Federation pool, provider and deploy service account with `run.admin`, `iam.serviceAccountUser`, `artifactregistry.writer`, `secretmanager.secretAccessor` | n/a | Stephen | ✅ DONE | n/a | `gcloud config get-value project` prints `hold-2026`; billing active; WIF provider id recorded in Notes | 2026-09-03 00:25 Stephen: project hold-2026 (number 1092063300634) under the trial account, billing linked, APIs on, Artifact Registry hold, deploy SA hold-deploy@hold-2026.iam.gserviceaccount.com, WIF provider projects/1092063300634/locations/global/workloadIdentityPools/github/providers/hold-repo bound to StephenSook/hold, seven Secret Manager entries (Confluent ones seeded unset until 0.3), four GitHub secrets; scripts/gcp_setup.sh is the record |
| 0.2 | Bob trial signups for BOTH people through the hackathon IBM resources link; plan tier and Bobcoin balance recorded here; upgrade to Pro+ or budget overage ($0.50 per coin) | n/a | both | 🟡 WIP | n/a | Both gauges visible; balances logged in Notes | WIP 2026-09-03 13:10 Stephen: Stephen's side recorded: three trial accounts through the hackathon link, each at the 50-coin trial tier and each at 50 of 50 spent by the end of Phase 2 (bobcoins-stephen-20260902-p1.png, -p1-account2.png, -p2-account3.png); no upgrade bought; Deem's row is Deem's |
| 0.3 | Confluent Cloud signup through the hackathon link; Basic cluster in the Cloud Run region; API key created; stored in Secret Manager only; Confluent CLI installed | n/a | Stephen | ✅ DONE | 0.1 | `confluent kafka topic list` works; no key in the repo | 2026-09-03 10:49 Stephen: Confluent Cloud account (Google sign-in, $400 trial, card on file), cluster hold (Basic, GCP us-central1, lkc-k8kp7w6, bootstrap pkc-n3603.us-central1.gcp.confluent.cloud:9092), API key DRYXZLI7GZNN5GWZ with its secret in Secret Manager only, topics hold.set-events and hold.verdicts, Confluent CLI installed; Cloud Run revision hold-00028-zt9 mounts the versions |
| 0.4 | Devpost project draft, track IBM, Deem invited as team member | n/a | Stephen | ✅ DONE | n/a | Draft visible to both | DONE 2026-09-03 13:05 Stephen: draft at https://devpost.com/software/hold-wrp4nq (project 1415567, draft submission 1169291 to agentic-cinema); write-up rewritten in Stephen's voice on the Pitch Arc (project version 2); tagline, built-with and links set; the track and the custom questions are answered at submit time (6.3). Deem's invite: the API cannot add members and no email is on file (the GitHub invite went to the collaborator already on the repository), so the private join link on the edit page is forwarded to Deem by Stephen |
...
| 0.11 | App Store Connect: confirm the Developer Program is active and record the Team ID; bundle id `com.stephensookra.hold`; app record; `ITSAppUsesNonExemptEncryption` NO; Test Information (description, feedback email) filled | n/a | Stephen | ✅ DONE | n/a | App record exists; Team ID in Notes | DONE 2026-09-03 13:55 Stephen: Developer Program active, Team ID Y8XVCMLQCZ; App ID `com.stephensookra.hold` (explicit) registered; App Store Connect record created as "HOLD Schedule" (the name HOLD is taken on the store; the bundle id is unchanged), Apple ID 6808303237, SKU hold-2026, iOS, English (U.S.), full access; TestFlight Test Information saved (beta description, feedback email, marketing URL = the repo). The Beta App Review contact block (name, phone, email) is filled and verified after a page reload; the number itself is not recorded in this repository. `ITSAppUsesNonExemptEncryption` NO is an Info.plist key and lands with the Capacitor iOS project in 3.8 |
| 0.12 | Day-0 usage screenshots, both accounts (gauge, Settings > General) | `docs/bob-evidence/bobcoins-stephen-20260902-p0.png`, `docs/bob-evidence/bobcoins-deem-20260902-p0.png` | both | 🟡 WIP | 0.2 | Files committed, identifiers redacted, figures and dates intact | WIP 2026-09-03 13:10 Stephen: no day-0 capture exists for Stephen's accounts; the earliest are the exhaustion captures on 2026-09-02 (p1, p1-account2), the same calendar day the build started, and that absence is stated here rather than reconstructed; Deem's capture is Deem's |
...
| 1.10 | Stub deploy, not gated on the residual: `api/main.py` (plain FastAPI; API routes first; catch-all GET serves `web/dist/index.html` for non-`/api` paths; CORS for `capacitor://localhost`, `http://localhost`, `https://localhost`, the Cloud Run origin); `Dockerfile` at repo root (Node stage builds `web/`, Python stage copies `web/dist`); `deploy.yml` with `--no-cpu-throttling --min-instances=1 --max-instances=1 --cpu=2 --memory=2Gi --concurrency=80 --timeout=3600 --session-affinity`; verify from a logged-out browser (fresh-project GFE 404 trap) | `api/main.py`, `Dockerfile`, `.github/workflows/deploy.yml` | Stephen | ✅ DONE | 0.1, 0.9, 0.10 | Public URL serves the board; `/api/status` 200 with a stub; `/judge` deep link loads | 2026-09-03 00:30 Stephen: live at https://hold-fwmdq7fc3q-uc.a.run.app (revision hold-00003-t2l from 1ebf11f through deploy.yml and Workload Identity); /api/status 200 with the FACTS headline, /api/docs 200, the root serves the placeholder page until web/ exists; verified from this Mac, a logged-out browser check is 3.7 |
...
| 2.15 | Phase 2 boundary: session-store export, screenshots both accounts, ATTRIBUTION regenerated | `docs/bob-evidence/**` | both | 🟡 WIP | 0.13 | Committed with dates | WIP 2026-09-03 13:10 Stephen: our half done: session-store export committed (bob-usage-evidence.json, 6 tasks, $125.49), Stephen's three screenshots, ATTRIBUTION regenerated at every boundary since; Deem's screenshots are Deem's |
...
| # | Task | Files | Owner | Status | Deps | Done means | Notes |
|---|---|---|---|---|---|---|---|
| 3.1 | ADK agent: `root_agent = LlmAgent(model=GEMINI_MODEL, output_schema=ExtractResult, tools=[check_legality, optimize_schedule, lookup_rule])` on google-adk 2.6.3 (output_schema and tools together is documented for this version); `GOOGLE_GENAI_USE_ENTERPRISE=True`; the three tools wrap 2.7 (or 2.9 if 2.7 was cut), 2.8 and 2.1 | `api/agents/hold_agent/agent.py`, `api/agents/hold_agent/__init__.py` | Stephen | ✅ DONE | 0.15, 2.8, 2.9 | The sample call sheet PNG extracts into a valid `ExtractResult` with `status: ok` | 359c9cd, 2026-09-03 01:05: live on Cloud Run through Vertex (gemini-3.1-flash-lite, global location); the sample call sheet extracts to status ok in 3.3 s, golden at data/fixtures/extraction/callsheet-day3.expected.json; extraction runs the tool-less twin hold_extract, root_agent keeps the tools |
| 3.2 | Structured extraction: multimodal (PNG, PDF, text) into `ScheduleInput`; natural-language constraints ("actor C unavailable days 1 to 3", "scene 7 before scene 2") into typed constraints; ambiguity returns `needs_clarification` with questions, never a guess; golden-file tests on three sample documents; under `HOLD_FAKE_EXTERNALS=1` extraction returns the golden fixture without calling Gemini | `api/agents/hold_agent/prompts.py`, `data/demo/samples/*`, `api/tests/test_extract.py` | Stephen | ✅ DONE | 3.1 | Three golden files pass; an ambiguous sample returns questions; fake mode returns the fixture | ea2f5ae, 2026-09-03 01:08: three goldens recorded live (call sheet, plain-English constraints into typed constraints, ambiguous note refused with questions); PNG and PDF inputs accepted by the route (image_base64 + mime_type) but only text goldens are recorded; network-marked tests run by hand |
| 3.3 | Guardrails: `before_tool_callback` allowlists the three tools and validates arguments with Pydantic; the agent never accepts a schedule (human Confirm in the UI); `/api/extract` has a 30 s timeout and one Gemini call per request | `api/agents/hold_agent/callbacks.py` | Stephen | ✅ DONE | 3.1 | An invalid tool argument is refused with a named field | 18584fc, 248 tests, 2026-09-02 23:54; allowlist and Pydantic validation refuse with a named field; 30 s timeout and max_llm_calls=3 in the runner (one call raised LlmCallsLimitExceededError live: tools plus an output schema take a thought turn and a structured final turn) |
| 3.5 | Routes: `POST /api/solve` (ThreadPoolExecutor, one worker; never the event loop), `GET /api/jobs/{id}`, `GET /api/events` (SSE; objective values via `loop.call_soon_threadsafe` from the solution callback; verdict updates), `POST /api/extract` (ADK `Runner` + `InMemorySessionService`; the ADK API server is never mounted), `POST /api/set-events` (in-process event bus, which is also the `HOLD_FAKE_EXTERNALS=1` path; Confluent producer added in 4.1), `GET /api/rules`, `GET /api/bench`; verify `/api/status` answers during a solve | `api/routes/**`, `api/hold/bus.py`, `api/tests/test_routes.py` | Stephen | ✅ DONE | 2.9, 3.2 | Full loop over HTTP locally including an in-process set-event re-solve; status answers mid-solve | 1182cb3, 240 tests, 2026-09-02 23:43; the live /api/extract path waits on 3.1 (503 until then, fixture under fake externals); Confluent leg is 4.1 |
| 3.6 | `GET /api/status`: computed once at startup, cached 10 minutes with `computed_at`; headline from `data/demo`; `benchmark_matched` from `bench/results.json` with its run SHA; `runtime {gemini_model, gemini_location, adk_version, ortools_version, confluent {connected:false ...}, mode}`; `scripts/facts.py` writes `docs/FACTS.json`; `api/tests/test_facts.py` checks the eight headline fields in README and docs (digits and spelled-out numerals) against FACTS, with an explicit allowlist for rules-sourced figures and citations | `api/routes/status.py`, `scripts/facts.py`, `docs/FACTS.json`, `api/tests/test_facts.py` | Stephen | ✅ DONE | 3.5 | Test green; FACTS has every headline field | 242bed8, 235 tests, 2026-09-02 23:21; headline hold days 4 to 0, illegal days 1 to 0, $4,069.92 removed, 8/8, OPTIMAL; status route mounted ahead of the rest of 3.5 |
| 3.7 | Production deploy with the 1.10 flags, Secret Manager env, Application Default Credentials only; previous revision id recorded in Notes before each deploy; rollback command tested once; served bundle verified from a logged-out browser | `.github/workflows/deploy.yml` | Stephen | ✅ DONE | 3.5 | Live URL runs the loop end to end; rollback verified | 2026-09-03 01:11 Stephen: deploy.yml through Workload Identity on every push (no key file; Secret Manager env; the runtime identity is the compute service account with secretAccessor and aiplatform.user). Live loop verified with scripts/simulate_set_day.py against the URL (baseline OPTIMAL, actor_late, scene_dropped and weather_cover each re-solved). Rollback tested: `gcloud run services update-traffic hold --region us-central1 --to-revisions hold-00019-k7b=100` served /api/status 200, then `--to-latest`. Before a risky deploy record the serving revision: `gcloud run services describe hold --region us-central1 --format='value(status.latestReadyRevisionName)'` (hold-00021-jhp at this note). Logged-out browser pass done 2026-09-03 11:14: `docs/screenshots/day0-api-docs-live-20260903.png` and `day0-api-status-live-20260903.png` captured from the served URL (0a7d244, e659c58); the status note that still said 3.5 and 4.1 were pending was corrected at 6d16339. Outage 2026-09-03 13:17 to 13:38 ET: a status field read a file the image did not carry (see 5.3 round six note); the uptime workflow caught it on its half-hourly run and the fix deployed as hold-00059-sjp |
...
| 3.13 | Phase 3 boundary: export, screenshots both accounts, ATTRIBUTION; `/api/status.bob_usage` serves the committed aggregate | `docs/bob-evidence/**`, `api/routes/status.py` | both | 🟡 WIP | 0.13, 3.6 | Endpoint matches the committed JSON | WIP 2026-09-03 13:10 Stephen: our half done: `/api/status.bob_usage` serves the committed export aggregate and the ATTRIBUTION counts (test_bob_usage.py), ATTRIBUTION regenerated (161 commits, 25 with the trailer, last f00aa11), Stephen's three account screenshots committed; Deem's screenshots are Deem's |
...
| # | Task | Files | Owner | Status | Deps | Done means | Notes |
|---|---|---|---|---|---|---|---|
| 4.1 | Confluent: topics `hold.set-events`, `hold.verdicts` (JSON, `rules/events.schema.json`); producer on `POST /api/set-events` (actor-late, scene-dropped, weather-cover); background consumer thread (auto-reconnect, `auto.offset.reset=latest`, log-and-skip on bad messages) re-solves and produces the verdict; SSE fan-out; `connected` true only after a real metadata call; in-process bus stays as fallback and `/api/status` reports which is live; `HOLD_FAKE_EXTERNALS=1` uses the bus and fixture extraction | `api/hold/streaming.py`, `api/routes/events.py`, `rules/events.schema.json`, `api/tests/test_streaming.py` | Stephen | ✅ DONE | 3.7, 0.3 | Publishing from the UI yields a new verdict on screen in under 5 s (measured); test green against the in-process bus | 2026-09-03 10:49 Stephen: live at 0d596b3 plus the status fix; /api/status reports transport confluent, connected true; measured: an event published straight to hold.set-events came back as a verdict on hold.verdicts in 0.7 s (a re-solve of the demo); the API mirrors its own set events and verdicts; in-process bus remains the fallback |
| 4.2 | `scripts/simulate_set_day.py`: labeled simulation publishing a day's events at speed for the video | `scripts/simulate_set_day.py` | Stephen | ✅ DONE | 4.1 | Runs against the live cluster | DONE 914bd63 2026-09-03 11:25 Stephen: `--transport confluent` produces each event on hold.set-events (no job_id) and reads the re-solve's verdicts from hold.verdicts, pass-2 numbers from the named job over HTTP. Live run against the deployed API and the `hold` cluster: actor_late 839.8 ms round trip with 4 verdicts on the topic, scene_dropped 455.8 ms with 3, weather_cover 571.1 ms with 2; every plan OPTIMAL, 0 hold days, 0 illegal days. HTTP transport at 09b991a |
...
| 2.11 | SAG-AFTRA penalties, second half: 12-hour rest, forced call lesser of daily rate or $900 (weekly $950), 56-hour weekend rest, meal within 6 hours, meal ladder $25 / $35 / $50 per half hour (flat $25 Ultra Low); integer cents | `rules/sag_rates.yaml`, `api/hold/penalties.py`, `api/tests/test_penalties.py` | Stephen | ✅ DONE | 2.5 | Hand calculation matches to the cent | DONE f98db45 2026-09-03 12:50 Stephen: seven penalty records (kind: penalty, never a minor's rule), sources captured from sagaftra.org through Chrome and a rendering fetch because the site answers 403 to scripts (headers state the capture), the Help Center answer by script with its sha256; forced call = lesser of day rate and $900 / $950, meal ladder $25 / $35 / $50 per half-hour or fraction, Ultra Low flat $25, 12-hour and 56-hour rest checks; 70 records all VERIFIED |
| 1.11 | Self-referential MCP server (`solve_schedule`, `check_legality`, `lookup_rule`, `run_residual` over stdio) registered in `.bob/mcp.json`; Bob calls `run_residual` from a Plan session | `api/hold/mcp_server.py`, `.bob/mcp.json` | Stephen | 🟡 WIP | 1.4 | Bob task log shows the call; export re-run and committed | WIP 2026-09-03 14:10 Stephen: server built (api/hold/mcp_server.py on mcp 2.1.1 over stdio, four tools, registered in .bob/mcp.json, driven by test_mcp_server.py through the protocol). The Bob half (Bob calls run_residual from a Plan session, export re-run) cannot happen: every Bob account was exhausted before the server existed; README and AGENTS state that |
| 4.5 | Optional: Cloud SQL Postgres for schedules and ADK `DatabaseSessionService`; only after 4.1 is closed | `api/hold/store.py` | Stephen | ✂️ CUT | 4.1 | Survives a restart | CUT 2026-09-03 17:10 Stephen: optional from the start. Nothing in the judged path needs a database: the job store is in memory by design behind a single Cloud Run instance (D11), the streaming leg is built on that, and adding a store now would be new stateful code on the judged path for no claim. Recorded rather than left open |
...
| # | Task | Files | Owner | Status | Deps | Done means | Notes |
|---|---|---|---|---|---|---|---|
| 5.1 | Wired-or-cut audit: grep the shipped code for every tool, model and vendor named in README, `/judge`, `.env.example`, `AGENTS.md` and the submission draft; cut or reword until claimed equals invoked | `docs/claims-audit.md` | Stephen | ✅ DONE | 3.7 | Audit committed; zero unwired claims | DONE 2026-09-03 16:20 Stephen: docs/claims-audit.md traces all fifteen claimed technologies to an import or a workflow line, names the two present-but-unclaimed (MiniZinc's instance format, and the vendor names inside the claims guard, which the audit deliberately does not repeat because the guard scans this file too) and maps the Devpost Built With tags to the same evidence; `.env.example` held by test_env_template.py, README and AGENTS at 35db32b, README paths by test_readme_paths.py |
| 5.2 | Claims test: models and vendors named on judge-facing surfaces are a subset of those `/api/status.runtime` reports; non-vacuity assertion; mutation-tested once each way; streaming phrased as "connected at submission time" | `api/tests/test_claims.py` | Stephen | ✅ DONE | 5.1 | Green; both mutations go red | b588cbe, 254 tests, 2026-09-03 00:05; both mutations red in the test file; re-run at 5.1 after the README rewrite |
| 5.3 | Independent adversarial review of the solver, legality and agent code (logic, not diff) by a second model (Gemini CLI) or a second reader; cap 3 rounds or 3 hours; every finding verified against source before a fix | n/a | Stephen | 🟡 WIP | 3.7 | Rounds and outcomes recorded in Notes | WIP 2026-09-03 00:45 Stephen. Round 1 (Codex, pass-1 range): 5 findings, all real, fixed. Round 2 (Codex, 5018122..1c5d82a): 6 findings, all real, fixed in 8dd9ebe, cdea6ab, f739477, 4089ae2, 2f1bbc0, 2bda6c8. Round 3 (Codex died on usage, Gemini CLI rate-limited, Grok delivered; 1c5d82a..40804fb): 4 HIGH findings, all real, fixed in adadf31, 5ce2487, 9480ecd and the calendar-gap commit; the P&H lead was refuted after 919ba94. Round 4 (Codex, 40804fb..60bfc61, 2026-09-03 10:56): 9 findings (2 high, 6 medium, 1 low), all verified real, fixed in seven commits (tool budget and extract hardening, number terminators, configured-extraction status and claims, ExtractResult contract, FACTS field, simulation exit, threat model). A fifth round on the round-4 fixes and the Confluent leg is next. Round five (2026-09-03 13:20, Grok on 60bfc61..ea5d0e2 after Codex's backend answered 404 twice): seven findings, all verified against source and real, one commit each: 09392da (set events chain on the latest schedule, solved or solving; the lost update), 60bf7b8 (a dead or unstarted consumer turns connected off), 07c9d02 (an event the handler cannot apply is skipped with its reason), 91ca989 (only a job this API minted is an echo; mirrored counted), 67047a7 (the response names the transport that carried the event), 2e6ff6a (the guard fails closed without state; no stop claim), 2cd5069 (group id asserted by value). 308 tests. Round six pending on 5732e5c..HEAD Round six (2026-09-03 14:40, Grok on 5732e5c..462b2db; Codex hit its usage limit twice): nine findings, all verified and real, one commit each: 316f2f2 (unknown rate tier refused), 80b5b8d (zero UNVERIFIABLE records asserted), b631ac4 (the call-sheet eval expectation is a valid ExtractResult with the stated $834 rate; eval re-recorded 4 of 4 at a133f2e), 8d53071 (consecutive-employment record carries the firm pick-up exception), 50003c9 (verdicts published before done; red proven with a slowed publish), 23c6ba4 (a failed solve keeps its edit in the chain), 31c7875 (base-read, edit and submit under one chain lock; the test errored rather than failed before the fix, so the red proof here is Grok's reproducer, not ours), a35a561 (the Confluent simulation attributes each step by the job's reported source), 4737237 (the eval recorder pairs the summary with its own run's result file; FACTS.adk_eval held to docs/adk_eval.json). Also this afternoon: a live outage from 13:17 to 13:38 ET, /api/status 500, caused by ff0aa4f reading docs/bob-evidence that .dockerignore excluded from the image; hotfix 1abd421 (tolerant reader, evidence text shipped, ignore rules tested against RUNTIME_FILES). 321 tests. Round seven pending on 462b2db..HEAD Round seven (2026-09-03 15:10, Grok on 462b2db..e55f318): six findings, all real, one commit each: 206b02a (run_residual crashed on optima.json's _note key; empty list refused), 8638dfd (notices regenerated after the MCP SDK; held to installed distributions), 88319de (24-hour pick-up leeway named as unmodeled), 21d477c (JUDGE.md events URL replays), 30d4ce3 (bus history appended on the publishing thread; red proven with the loop thread blocked), bd0f3de (eval prompt is the sample sheet verbatim, gold asks nothing; eval re-recorded after). 325 tests. Round eight pending on e55f318..HEAD Round eight (2026-09-03 17:05, Grok on e55f318..b7c9b7f): three findings, all real, one commit each: 1b54c68 (an event published between subscribe and replay was delivered twice; the bus numbers every event and the stream skips what it already replayed), 955ce0e (a failed job wrote its status before publishing, unlike the success path), ce94db4 (the recorded live goldens still carried the pre-b9c3309 ids while the agent answers with the documented convention: both re-recorded from live calls against the deployed instance, and the test now compares a cast member by letter and a constraint by scene number so a naming change is never read as an extraction error). 332 tests. Rounds one to eight have all returned real findings; the next round runs on ce94db4..HEAD |
| 5.4 | Security: gitleaks over full history, dependency supply-chain check, license guard (everything permissive), `.gitleaksignore` only with hand-verified fingerprints | `.gitleaksignore`, `docs/THIRD_PARTY_NOTICES.md` | Stephen | ✅ DONE | 0.9 | Clean | DONE 2026-09-03 13:10 Stephen: gitleaks 8.30.1 over the full history (165 commits) found no leaks, so no .gitleaksignore; docs/THIRD_PARTY_NOTICES.md generated from the installed metadata (all permissive; google-crc32c's metadata carries no string, its wheel LICENSE is Apache-2.0); the license guard runs as a CI job on every push, and Socket posts its report as its own check run from the GitHub App rather than from a workflow |
| 2.16 | Quote verification, fetching version: `scripts/verify_quotes.py` fetches each `source_url` into the gitignored cache (paced 8 to 10 s, content-checked, never trusting a 200), refreshes `rules/sources/` snapshots and `rules/verification.json` | `scripts/verify_quotes.py`, `rules/verification.json` | Stephen | ✅ DONE | 2.10 | Record complete; any UNVERIFIABLE named | DONE 54c936e 2026-09-03 16:10 Stephen: scripts/verify_quotes.py refetches every distinct source_url (24 of them, all snapshotted) into the gitignored cache at eight-second pace and judges the body, never the status: a PDF must start with %PDF, a short or challenge-stub HTML body is refused, a body missing the record's quote is drift. --write refreshes one snapshot only after an unchanged fetch. First full run recorded in the Notes below Run committed as docs/quote-refetch.md: 21 of 24 sources unchanged against the live page, 3 blocked (the sagaftra.org pages, as their headers record), no drift. The first run's one drift and four errors were the checker's own (an encoded ampersand; a certificate chain this machine cannot complete without the system trust store), fixed at 4ed4581 and c3bb1f4 |
| 3.4 | adk eval: three or four text cases (extraction, NL constraint, refusal on ambiguity, tool trajectory), `tool_trajectory_avg_score` plus `final_response_match_v2`; an image case is added only if the local runner accepts an image part (try one); run once with credentials; score into FACTS via `scripts/facts.py` | `api/agents/hold_agent/evalset.json`, `api/agents/hold_agent/test_config.json` | Stephen | ✅ DONE | 3.6 | Score in FACTS | DONE b1de22e 2026-09-03 12:12 Stephen: 4 of 4 cases PASSED, tool_trajectory_avg_score 1.0 and final_response_match_v2 1.0 on every case, recorded by `scripts/adk_eval.py` from ADK's own result file into `docs/adk_eval.json` and FACTS `adk_eval`. Two earlier runs failed three cases on the 500-call ceiling: with tools and an output_schema together, gemini-3.1-flash-lite re-called lookup_rule forever (live trace); the tool agent now answers free-form and the extraction twin keeps the schema (5732e5c). The image case was not added: text cases only. Eval set at 0973197. Re-recorded three times on 2026-09-03 afternoon after round six and seven findings: the gold gained the stated $834 rate (2 of 4 once the prompt became the sample sheet: the tool agent ran check_legality on a complete sheet), the instruction now opens and closes with the no-tool extraction rule (3 of 4: rate_tier "scale" and free-form ids), then names the four tiers and the id convention (4 of 4, every metric 1.0). facts.py holds every "passes N of M cases" sentence to the record |
| 3.14 | `THREAT_MODEL.md` table (asset, threat, control, residual risk), including the unmounted ADK server and the extraction timeout | `docs/THREAT_MODEL.md` | Stephen | ✅ DONE | 3.3 | Committed | 2ca72c9, 2026-09-02 23:48; extraction controls listed as planned until 3.1 and 3.3 land, then the rows update |
| 5.5 | README to the judged shape: problem, what it does, the three proofs, architecture (only what `/api/status` self-reports), how to run, honesty panel, whether a practitioner interview was obtained, How IBM Bob was used, streaming section, license; `JUDGE.md` 90-second walkthrough | `README.md`, `JUDGE.md` | Stephen | ✅ DONE | 3.6, 5.1, 5.12 | A fresh reader reproduces the residual from the README alone | DONE 462b2db 2026-09-03 13:30 Stephen: README rewritten to the judged shape (problem, what it does, three proofs from FACTS, architecture as /api/status reports it, quick start that reproduces the residual, honesty panel, streaming, Bob evidence, license); JUDGE.md is a nine-step API and repository walkthrough. The web app's steps are added when web/ lands. This row was left TODO by mistake until 2026-09-03 17:10: the status edit sat in a script that exited on a failing gate before it ran |
...
| 5.8 | `uptime.yml`: content-check `/api/status` for `benchmark_matched`, offset minutes, fails if the URL variable is unset | `.github/workflows/uptime.yml` | Stephen | ✅ DONE | 3.7 | Newest run age matches cadence | 2026-09-03 00:33 Stephen: uptime.yml at 17 and 47 past each hour plus manual dispatch; HOLD_URL repository variable set; watch the newest run's age against the cadence, not the green |
...
| 5.10 | Video beats captured as features land (1080p60, zoomed, real device for mobile, one unbroken take for offline with a visibly failing request); `docs/video/raw/` gitignored | `docs/video/script.md` | Stephen | 🟡 WIP | 3.7 | Every beat on disk | WIP 2026-09-03 14:50 Stephen: docs/video/script.md written on the seven-step arc with every number read from FACTS at recording time; API beats can be captured now, the web beats wait for Deem's app; no capture on disk yet |
| 5.11 | AI-tone and em-dash sweep on every judge-facing surface; PII sweep (`git ls-files` and eyeball) | n/a | Stephen | ✅ DONE | 5.5 | Clean | DONE 2026-09-03 16:30 Stephen: docs/sweeps.md carries the commands and the run: no dash, no AI-tone hit, no phone shape, no private material tracked; two deliberate exceptions named (a curly quote inside the normalizer's own test, two Google service-account addresses). The teammate handle came out of the 0.4 note. The sweep doc names its characters by code point, because a document holding them fails the gate it documents (it did, twice, before this landed). Re-run in the freeze window (7.1) |
| 5.12 | Bob `/review` on `api/hold/` and `api/agents/`, SARIF committed; `docs/bob-evidence/build-trace.md`; `api/tests/test_bob_usage.py`; lane-enforcement record with a real refusal SHA; exhaustion screenshots for any account at zero; honesty log for any row not captured; `/api/status.bob_usage` adds the build trace | `security/review-*.sarif`, `docs/bob-evidence/**`, `api/tests/test_bob_usage.py` | Stephen | 🟡 WIP | 4.7 | Green in CI; every evidence row has a file or a dated absence statement | WIP 2026-09-03 17:10 Stephen: what exists ships (test_bob_usage.py, /api/status.bob_usage, ATTRIBUTION with the build trace, lane-enforcement.md, three exhaustion screenshots) and docs/bob-evidence/honesty-log.md names the four rows that cannot be captured, all because every Bob account hit zero Bobcoins during Phase 2: the /review SARIF, a lane refusal with a real SHA, any later session export, and the half of 1.11 where Bob itself calls the MCP server |
...
| 6.3 | Devpost: every field, track IBM, repo URL, hosted URL, video URL, Google Cloud products, other tools, team count 2; SUBMIT by 21:00 ET; reload and re-read every field | n/a | Stephen | 🟡 WIP | 6.2, 5.5 | Project shows Submitted; every field re-read after reload | WIP 2026-09-03 17:05 Stephen: the Additional info step is filled and verified after a full reload (submitter type Team, organization N/A, no government employee, United States, Canada province N/A, project New, track IBM, team size 2, repo and hosted URLs, and the Google Cloud answer naming only what deploy.yml actually uses: Cloud Run, Vertex AI, Secret Manager, Artifact Registry, Workload Identity Federation and Cloud Logging, with the image built in CI rather than Cloud Build). One required field is deliberately left empty, "list all other tools or products you used": that answer is Stephen's to write, and nothing false will be written into it here. Submission stays in DRAFT; the Submit step waits on the video (6.1, 6.2) |
...
| 7.4 | Post-submission: calendar entries for Oct 1 (Confluent card before the trial ends) and daily uptime checks through Oct 8; links re-clicked weekly | n/a | Stephen | 🟡 WIP | 7.3 | Calendar entries exist | WIP 2026-09-03 17:25 Stephen: the five gate reminders exist on Stephen's calendar (Sep 5, Sep 6, Sep 8 21:00, Sep 9 12:30 PT, Oct 1). The Oct 1 one was written as "add a card before the trial ends", which was already done on Sep 3 when the cluster launched, so it now carries the predicate that is still open: check whether the promotional balance is gone and whether the cluster and `/api/status` still report connected, and cancel the task if settled. Trial inventory: the Confluent promotional balance runs out about Oct 2, inside judging (Sep 10 to Oct 8), with a card on file so the end of the balance is a charge and not an outage; Cloud Run is pinned to one always-on instance at 2 vCPU and 2 GiB (D7) and bills continuously against the GCP trial credit, which the same reminder checks. The daily uptime half stays open until after submission Stephen's call 2026-09-03: let the credits run out during judging and roll onto the card, then cancel everything afterwards if the project does not place; nothing is torn down while judging is open. Devpost key dates checked the same day: submissions close Sep 9 21:00 UTC, judging Sep 10 to Oct 8 19:00 UTC, winners announced Oct 13 19:00 UTC, so the teardown reminder is armed for Oct 14, not Oct 9, and it carries the order (Confluent cluster, then the Cloud Run service or the whole project, then a billing check a week later) and the instruction to disable the uptime workflow in the same sitting so a dead service does not train anyone to ignore alert mail |
...
| `ExtractResult` | Stephen | Deem, agent | `{status: ok or needs_clarification, schedule: ScheduleInput or null, questions: list[str], notes: str}`; Gemini-friendly (Literal enums, no free-form dicts); validated: `ok` carries a schedule, `needs_clarification` carries questions and no schedule (announced 2026-09-03 03:05, lands under a CONTRACT commit) |
