...
| 2.8 | Pass 2: rules as plain hard constraints on the extended model, `symmetry_break=False`, holding objective plus penalty terms, `num_workers = os.cpu_count()` in production, time limit, OPTIMAL vs FEASIBLE with bound; on INFEASIBLE re-run with rule literals as assumptions and return UNDETERMINED with reasons | `api/hold/solve.py`, `api/tests/test_pass2.py` | Stephen | WIP | 2.7, 1.3 | `data/demo/hold-demo.json` returns 0 hold days, 0 illegal days; the two panels are separate fields | WIP 2026-09-02 22:26 Stephen; pass 2 lives in api/hold/pass2.py, solve.py keeps pass 1 and assembles SolveResult; cost model reads overnight_location |
