...
| 5.12 | Bob `/review` on `api/hold/` and `api/agents/`, SARIF committed; `docs/bob-evidence/build-trace.md`; `api/tests/test_bob_usage.py`; lane-enforcement record with a real refusal SHA; exhaustion screenshots for any account at zero; honesty log for any row not captured; `/api/status.bob_usage` adds the build trace | `security/review-*.sarif`, `docs/bob-evidence/**`, `api/tests/test_bob_usage.py` | Stephen | 🟡 WIP | 4.7 | Green in CI; every evidence row has a file or a dated absence statement | WIP 2026-09-03 17:10 Stephen: what exists ships (test_bob_usage.py, /api/status.bob_usage, ATTRIBUTION with the build trace, lane-enforcement.md, three exhaustion screenshots) and docs/bob-evidence/honesty-log.md names the three rows that cannot be captured, all because every Bob account hit zero Bobcoins during Phase 2: the /review SARIF, a lane refusal with a real SHA, and any later session export |
