...
| 2.5 | SAG-AFTRA rates, hold-day half: Low Budget $810/day and $2,812/week, Moderate Low $449.05, Ultra Low $249, P&H 21% rising to 22% on 2026-09-06, hold day at full rate; integer cents, stated rounding; every figure with source URL and cycle dates, re-verified against the ratified 2026 agreement before it renders | `rules/sag_rates.yaml`, `api/hold/penalties.py`, `api/tests/test_penalties.py` | Stephen | DONE | 2.1, 1.12 | A hand calculation on `data/demo/hold-demo.json` matches the code to the cent | 650b30f, 7 tests |
...
| 2.10 | Quote check, Thursday version: substring test of every `quote` against a committed text snapshot per source under `rules/sources/` with a hash; `rules/verification.json`; CI test fails (never skips) if any record lacks a status | `rules/sources/**`, `rules/verification.json`, `api/tests/test_quotes.py` | Stephen | WIP | 2.4 | CI log shows PASSED; UNVERIFIABLE records labeled, not counted | WIP 2026-09-02 21:35 Stephen; first live fetch shows the GA hour tables live on a Department form, not in the rule text, and the CA wage-order URL is a 404: quotes and citations get rewritten from the fetched text |
...
| `Verdict` | Stephen | Deem | `{status: LEGAL or ILLEGAL or UNDETERMINED, day, violations: [{rule_id, citation, title, limit, computed, over_by, quote, source_url, jurisdiction}], core_rule_ids, witness}`; the checker fills `violations`, the solver fills `core_rule_ids`; `witness` (LEGAL only) keys: `day, date, crew_call, crew_wrap, heuristic, scenes: [{id, start, end, cast_ids}], minors: {cast_id: {call, dismiss, work_minutes, location_minutes, meal: {start, end} or null}}`, times as `HH:MM` |
