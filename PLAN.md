...
| 0.9 | CI skeleton: api (ruff, mypy, pytest hermetic, `fetch-depth: 0`), web (tsc, eslint, `vitest run`, build), gitleaks, em-dash gate over `git ls-files --cached --others --exclude-standard`, license guard; every gate bare; required checks set on `main` | `.github/workflows/ci.yml`, `scripts/no_em_dash.sh` | Stephen | WIP | 0.5, 0.8, 0.10 | Green on main; a planted em-dash in an untracked file turns it red; branch protection lists the jobs | n/a |
