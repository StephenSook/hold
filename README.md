...
| Android 7.0 or newer. Allow installs from this source when prompted. | Needs the TestFlight app. Build 3 is uploaded and waiting on Apple's beta review, so until that clears the link reads "this beta isn't accepting any new testers right now". The APK on the left needs no such wait. |
